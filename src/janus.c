...
gint main(int argc, char *argv[]) {
	...
	gboolean turn_rest_api_tls_verify_certificate = TRUE;
#endif
...
#ifdef HAVE_TURNRESTAPI
	item = janus_config_get(config, config_nat, janus_config_type_item, "turn_rest_api_tls_verify_certificate");
	if(item && item->value)
		turn_rest_api_tls_verify_certificate = janus_is_true(item->value);
...
#else
	if(janus_ice_set_turn_rest_api(turn_rest_api, turn_rest_api_key, turn_rest_api_method, turn_rest_api_timeout, turn_rest_api_tls_verify_host, turn_rest_api_tls_verify_certificate) < 0) {
